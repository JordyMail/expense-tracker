// frontend/src/features/transactions/components/TransactionInput.jsx
import React, { useRef, useEffect } from 'react';
import { ACCOUNT_OPTIONS, CATEGORY_OPTIONS, FORM_FIELDS } from '../../shared/utils/constants';
...
const TransactionInput = ({
  ...
  const handleAccountChange = (e) => {
    ...
    const matchedOption = ACCOUNT_OPTIONS.find(opt => opt.name.toLowerCase() === value.toLowerCase());
    if (matchedOption) {
  ...
  const handleCategoryChange = (e) => {
    const value = e.target.value;
    const matchedOption = CATEGORY_OPTIONS.find(opt => opt.name.toLowerCase() === value.toLowerCase());
    if (matchedOption) {
 ...
 const handleAccountKeyDown = (e) => {
    // Jika dropdown terbuka, gunakan handler dari useDropdown
    if (showAccountDropdown) {
      const handled = handleAccountDropdownKeyDown(e);
      if (handled) return;
    }
    
    // Jika dropdown tertutup dan user tekan panah bawah, buka dropdown
    ...
    // Selain itu, passing ke parent keyboard handler
    onKeyDown(e);
  };

  // Handler untuk category field
